...
<div class="breadcrumb">
    <ul>
        ...
        <li class="last">View Ownership Change Request</li>        
    </ul>
</div>
<!-- end breadcrumb here-->
...
<?php echo $this->Form->create('VehicleRegistrationCompany', array('url' => array('controller' => 'vehicles', 'action' => 'companysubmit'), 'type' => 'file','enctype'=>'multipart/form-data')); ?>
           <table> <tr>
				...
				<td width="16%"><?php
                        echo $this->Form->input('VehicleDetail.nu_company_id', array('label' => false,
                            ...
                            'onchange' => "formsubmit('VehicleRegistrationCompanyOwnershipchangedetailForm');",
                            'maxlength' => 30,
                            'class' => 'round_select')
                        );
                        ?></td>
    ...
    <!-- end innerbody here-->
    <h3>View Ownership Change Request</h3>
    <!-- Start innerbody here-->
    <div class="innerbody">
        <table width="100%" cellspacing="1" cellpadding="5" border="0" >
            <tr class="listhead vehicleRegsPag">
                <td width="">SI.No.</td>
                <td width="13%">Change <br/>Requested date</td>
                <td width="">Requested Name</td>
                <td width="">Change Request type</td>
                <td width="">Request Status</td>
            </tr>
            <?php if (count($ownershipchangedetails) > 0) : ?>	
                <?php
                $i = 1;
                foreach ($ownershipchangedetails as $key => $showdata) : $sr = $i % 2 == 0 ? '' : '1';
                    ...
                    <tr class="cont<?php echo $sr; ?>">
                        ...
                        <td><?php echo date('d M Y',strtotime($showdata['NameChangeHistory']['dt_amend_date']));
						
						
						?></td>
                        <td ><?php echo ucfirst($showdata['NameChangeHistory']['vc_customer_name']);
						//echo $globalParameterarray[$showdata['NameChangeHistory']['vc_amend_type']]; ?></td>
                        <td ><?php echo $globalParameterarray[$showdata['NameChangeHistory']['vc_amend_type']];

 if($showdata['NameChangeHistory']['ch_doc_upload']=='Y'){

									//echo $globalParameterarray[$val['Company']['vc_bank_supportive_doc']];			

									echo '<br>';
									
									$url =$this->webroot."vehicles/downloadammned/N/".base64_encode($showdata['NameChangeHistory']['vc_cust_amend_no']);
						...
						?></td>
                        <td ><?php echo $globalParameterarray[$showdata['NameChangeHistory']['ch_approve']]; 
						if($showdata['NameChangeHistory']['ch_approve']=='STSTY05'){
						echo '<br/>';
						echo '<b>Reason :</b> '.$showdata['NameChangeHistory']['vc_remarks'];
						}
						?></td>
                    <tr>
