            ...
            $.ajax({
                ...
                if (data !== '') {
					...
					if($('#ClaimCheckbox').is(':checked')){
						...
						$('#updoc'+rowCount).css('display','none');
					
						for(var i=0;i<=rowCount+1;i++){
							$('#ClaimMultiple').css('display','');
							$('#updoc'+i).css('display','none');
						}
						...
						for(var i=0;i<=rowCount+1;i++){
							$('#updoc'+i).css('display','');
							$('#ClaimMultiple').css('display','none');
						}
					}
...
var row = $('#table_row_claims_id').find('tr').length;
...
//alert(errroMessage);
		if(errroMessage == false){
			if($(obj).valid()==true){
				$('#submitclaimid').attr('disabled', 'disabled');
				$('#ClaimFlrEditForm').submit();
				return true;
