   ...
   function setDatePicker() {
        ...
        $(addobject).find("input[name*='dt_invoice_date']").datepicker({
            ...
            onSelect: function() {
				...
				console.log(invoicealreadycheck+'==invoicealreadycheck=setdatepicer===1=='+noOfdays+'==ltrValue=='+ltrValue);
...
/*				if(invoicealreadycheck == false){
				...
				console.log(element+'great+=='+value);
					getUniqueInAll(value,element);
				}*/
   ...
   function amountvalueAdmin(curentNuLitresvalue,noOfdays,litrescurrIndex,rejection){
                ...
                var nu_refund_rate = $("#ClaimDetail" + litrescurrIndex + "NuRefundRate").val();
					...
					console.log(litrescurrIndex+'==litrescurrIndex=2=admin_fee=='+admin_fee+'=='+amount);
					...
					console.log(litrescurrIndex+'==litrescurrIndex=1=admin_fee=='+admin_fee+'=='+amount);
   ...
   function getUniqueInAll(value,element){
		...
		if ($.trim(value) != '') {
				...
				if (litrescurrIndex != runObjIndex) {
                  ...
                  if (($.trim(notcurentVcOutletCodevalue) == $.trim(curentVcOutletCodevalue))
					...
					if(curentNuLitresvalue < 200 && noOfdays<90){
						...
						$("#ClaimDetail" + litrescurrIndex + "VcReasons").val('Invoice already exsist. and Fuel litres  less than 200 Ltrs.');
					...
					}else if(curentNuLitresvalue > 200 && noOfdays==0){
					...
					console.log(noOfdays+'=='+curentNuLitresvalue+'=getuniuqe=5=litrescurrIndex=='+litrescurrIndex);	
                    amountvalueAdmin(curentNuLitresvalue,noOfdays,litrescurrIndex,rejection);
                	
                    returnpass = false;
									
					//alert('inside444'+returnpass);
				...
				if (outletcurrIndex != runObjIndex) {
                  ...
                  if (($.trim(value) == $.trim($(this).val()))
				    ...
				    console.log(noOfdays+'=='+curentNuLitresvalue+'=getuniuqe=6=litrescurrIndex=='+outletcurrIndex);	
				...
				if (datecurrIndex != runObjIndex) {
                       ...
                       $("#ch_rejected_td_" + datecurrIndex).html('<img alt="" src="'+GLOBLA_PATH+'/img/with-check.jpg">');
						...
						console.log(noOfdays+'=='+curentNuLitresvalue+'=getuniuqe=7=litrescurrIndex=='+datecurrIndex);	
				...
				if (currIndex != runObjIndex) {
						...
						console.log(noOfdays+'=='+curentNuLitresvalue+'=getuniuqe=8=litrescurrIndex=='+currIndex);	
	...
	/*
	$(obj).find("input[name *='vc_invoice_no']").each(function() {
	$(this).rules("add", {
                required: true,
                alphanumeric: true,
                maxlength: 15,
   ...
   function checkAjax(Rowid,ltrValue,InvoiceDate){
		$.ajax({
				...
				if(data==false){
		             ...
		             console.log(noOfdays+'=='+curentNuLitresvalue+'=getuniuqe=98=litrescurrIndex=='+Rowid);	
                     amountvalueAdmin(ltrValue,'',Rowid,invoicealreadycheck);                	
            		 totalamountofinvoicesInfun();
					 console.log(invoicealreadycheck+'===111invoicealreadycheck_L==value=='+value+'==element=='+element);
					
					}
				else {
					 console.log('===222invoicealreadycheck_L==againvalue=='+value+'==element=='+element);
					 var invoicealreadycheck=true;
					 
					 if(Rowid ==0){
					 
						console.log(ltrValue+Rowid+InvoiceDate+'===66662invoicealreadycheck_L==againvalue==');
						callAfterajax(ltrValue,Rowid,InvoiceDate,invoicealreadycheck);
						totalamountofinvoicesInfun();						
					 
					 }else{
					// alert(value+'--value');
					  //checkmauli(value,element);
					  $getUniqueInAllrows =  getUniqueInAll(value,element);
				
					  totalamountofinvoicesInfun();
					 // alert('show retrun=='+$getUniqueInAllrows);
					  if($getUniqueInAllrows==true){
					    console.log(ltrValue+Rowid+InvoiceDate+'===78888invoicealreadycheck_L==againvalue==');
						callAfterajax(ltrValue,Rowid,InvoiceDate,invoicealreadycheck);
					    totalamountofinvoicesInfun();
					  }
					 }
  ...
  function callAfterajax(ltrValue,Rowid,InvoiceDate,invoicealreadycheck){
			...
			if(ltrValue ==0 && noOfdays <= 90){
				...
				console.log('new-callAfterajax2');
                $("#ch_rejected_td_" + Rowid).html('<img alt="" src="'+GLOBLA_PATH+'/img/with-check.jpg">');
			 ...
			 if (ltrValue < 200 && ltrValue!=0 && noOfdays > 90 ) {
				...
				console.log('--new-callAfterajax2111');
                $("#ch_rejected_td_" + Rowid).html('<img alt="" src="'+GLOBLA_PATH+'/img/with-check.jpg">');
				...
				console.log(ltrValue+'callAfterajax5111'+noOfdays);
			    $("#ch_rejected_td_" + Rowid).html('<img alt="" src="'+GLOBLA_PATH+'/img/without-check.jpg">');
                $("#ClaimDetail" + Rowid + "VcReasons").val('');
                $("#ClaimDetail" + Rowid + "VcReasonsDiv").html('');
                $("#showreason_id_" + Rowid).hide();
			...
			console.log(ltrValue+'===ltrValue====noOfdays='+noOfdays+'callAfterajax2111');		
		     return true;
	 }
