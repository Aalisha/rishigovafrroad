...
class InspectorsController extends AppController {
     ...
     *
     */
    var $uses = array('Inspector','CustomerLocationDistance');
                    ...
                    if ($detail) {
                        ...
                        if ($this->VehicleLogMaster->save($logdetails, false)) {
                            ...
                            foreach ($this->data['VehicleLogDetail'] as $key => $vehicleLogData) {
									...
									$checkRequest = $this->VehicleLogDetail->find('count', array(
                                         'conditions' => array(
									...
									$diffvar = $vehicleLogData['nu_end_ometer']-$vehicleLogData['nu_start_ometer'];
                                 ...
                                 // $vehicleLogData['dt_log_date'] = date('d-M-Y', strtotime($vehicleLogData['dt_log_date']));
                                    ...
                                    if ($vehicleLogData['selectedroad'] == 0) {
										...
										$nurdkmtavle= $vehicleLogData['nu_road_km_traveled'];
										//unset($vehicleLogData['nu_road_km_traveled']);
										
										$vehicleLogData['nu_road_km_traveled'] = $nurdkmtavle;
										
										
                                    } else {
									
                                        $vehicleLogData['vc_other_road_destination'] = $vehicleLogData['vc_destination'];
                                       
 									    $vehicleLogData['vc_other_road_orign']       = $vehicleLogData['vc_orign'];
										
										$nurdkmtavle   = $vehicleLogData['nu_road_km_traveled'];
										
										//unset($vehicleLogData['nu_road_km_traveled']);
										
										$vehicleLogData['nu_other_road_km_traveled'] = $nurdkmtavle;										 
                                    }
									...
									if($vehicleLogData['selectedroad'] == 1) {
									
									 $vehicleLogData['nu_other_road_km_traveled'] =$diffvar;
	...
	//	pr($sqlResult);
        ...
        foreach ($total as $amount) {
			...
			if($amount['AssessmentVehicleMaster']['vc_status']=='STSTY04'){
    ...
    function getvehiclelog() {
		...
		/*$arramax=array();
		$i=0;
		foreach($sqlResult['VehicleLogDetail'] as $value ){
		$arramax[$i]=$value['nu_end_ometer'];
		$i++;
		}
		echo max($arramax);
		die;*/
    ...
    public function vehiclelist() {
        ...
        try {
			...
			if (isset($this->params['named']['vehiclelicno'])) :
                ...
                $vehiclelicno = strtolower(trim($this->params['named']['vehiclelicno']));

            else :

                $vehiclelicno = (isset($this->data['Inspector']['vehiclelicno']) && !empty($this->data['Inspector']['vehiclelicno'])) ?trim($this->data['Inspector']['vehiclelicno']) :'';
    ...
    function vehiclelistpdf() {
        ...
        try {
			...
			if ($vehiclelicno) :
                ...
                $conditions += array(
                    ' lower(VehicleDetail.vc_vehicle_reg_no) LIKE' => strtolower(trim($vehiclelicno)) . '%'
                );
            ...
            $columnsValues = array('SI.No.', 'Customer Name',
                ...
                'GVM Rating', 'D/T Rating',
                'Rate(N$)');

            $this->Inspectorreportpdfcreator->headerData('Vehicle List Report', $period = NULL, $this->Session->read('Auth'), $toDate, $fromDate);

            $this->Inspectorreportpdfcreator->genrate_inspectorvehiclehistorypdf($columnsValues, $vehiclereport, $this->globalParameterarray, $this->Session->read('Auth.Profile'), $toDate, $fromDate, $vc_customer_name,$vehiclelicno);
    ...
    public function logsheet() {
        ...
        try {
			...
			if ($vehiclelicno):
                ...
                $conditions += array(
                    ' lower(VehicleLogDetail.vc_vehicle_reg_no) like ' => $vehiclelicno. '%' 
                );
...
//pr($vehiclelogreport);
    ...
    public function logsheetpdf() {
        ...
        try {
            ...
            $vc_customer_name = isset($this->data['Inspector']['vc_customer_name']) && !empty($this->data['Inspector']['vc_customer_name'])?trim($this->data['Inspector']['vc_customer_name']) :'';
            ...
            $columnsValues = array('Log Date', 'Customer Name', 'Vehicle Register No.',
            ...
            $this->Inspectorreportpdfcreator->genrate_inspectorvehiclelogsheet_pdf($columnsValues, $logreport, $this->globalParameterarray, $this->Session->read('Auth.Profile'), $toDate, $fromDate, $vc_customer_name,$vehiclelicno);
    ...
    function assessmenthistory() {
        ...
        try {
			...
			if ($vehiclelicno):
                ...
                $conditions += array(
                    ' lower(AssessmentVehicleDetail.vc_vehicle_reg_no) like ' => strtolower(trim($vehiclelicno)). '%' 
                );
    ...
    function assessmenthistorypdf() {
        ...
        try {
            ...
            $this->set('vc_customer_name', ucfirst($vc_customer_name));
            ...
            $this->Inspectorreportpdfcreator->genrate_inspectorassessmenthistory_pdf($columnsValues, $assessmentreport, $this->globalParameterarray, $this->Session->read('Auth.Profile'), $toDate, $fromDate, $vc_customer_name,$vehiclelicno);
    ...
    function vehiclelogsheet() {
        ...
        try {
			...
			if ($vehiclelicno) :
                 ...
                 $conditions += array(
                    'LOWER(VehicleLogDetail.vc_vehicle_reg_no) Like ' => strtolower(trim($vehiclelicno)).'%'
                );
    ...
    function vehiclelogsheetpdf() {
        ...
        try {
            ...
            $columnsValues = array('Log Date', 'Vehicle Register No.',
                'Vehicle LIC. No.', 'Driver Name',
            ...
            $this->Inspectorreportpdfcreator->genrate_inspectorvehicleloghistory_pdf($columnsValues, $vehiclelogreport, $vehicletype, $this->globalParameterarray, $this->Session->read('Auth.Profile'), $toDate, $fromDate,$vehiclelicno);
    ...
    function vehicleassessmentpdf() {
        ...
        try {
            ...
            $this->Inspectorreportpdfcreator->genrate_assessmenthistory_pdf($columnsValues, $assessmentreport, $vehicletype, $this->globalParameterarray, $this->Session->read('Auth.Profile'), $toDate, $fromDate,$vehiclelicno);
     ...
     *
     */
	 function getcustomerdetailbysearchreport(){
...
//die;	
    ...
    function getcustomerdetailbysearch() {
		...
		//pr($vehiclelist );
		//die;
		   foreach($vehiclelist as $key=>$VehicleDetail){
		  
			    $licenso_no=$key;
				if(in_array($licenso_no,$arraytransfervehicleslicenseno)==false){
					$arrayVehicleLogsLicense[$VehicleDetail]=$licenso_no;
				}
			}	
					 $this->set('vehicleList', $arrayVehicleLogsLicense);			
		...
		/*
		   ...
		   foreach($vehicleReg as $VehicleDetailreg){
            ...
            $this->set('vehicleReg', array('' => 'Select Registration. No.') + $arrayVehicleLogsRegno);
	...
	function inactivevehicles() {
		...
		//pr($this->data);die;
            ...
            if (!empty($this->data) && $this->RequestHandler->isPost()) {
				...
				foreach($this->data['VehicleLogDetail'] as $valuefile){
						...
						if($checkRequest = $this->CustomerInactiveVehicleLog->find('count', array(
                                            'conditions' => array(
                                                ...
                                                array('OR' => array(
                                                    array('CustomerInactiveVehicleLog.nu_start_ometer' => trim($valuefile['nu_start_ometer'])),
                                                    array('CustomerInactiveVehicleLog.nu_end_ometer' => trim($valuefile['nu_end_ometer']))
                                        ))))) > 0){
										
									$this->Session->setFlash('Odometer details of this vehicle already exist,please enter the unique value.', 'error');					    
									$this->redirect('inactivevehicles');
				...
				foreach($this->data['VehicleLogDetail'] as $value){
				 
					$this->CustomerInactiveVehicleLog->validate = null;
					
					$this->CustomerInactiveVehicleLog->create();
					
					$value['vc_vehicle_lic_no']=$this->data['VehicleLogDetail'][0]['vc_vehicle_lic_no'];
					$value['vc_vehicle_reg_no']=$this->data['VehicleLogDetail'][0]['vc_vehicle_reg_no'];
					
					//CH_ROAD_TYPE 0 means namibian road
					if($value['selectedroad']==0){
					
					$value['vc_orign']       = $value['vc_orign'];
					$value['vc_destination'] = $value['vc_destination'];
					$diff                    =  $value['nu_end_ometer']-$value['nu_start_ometer'] ;
					// unset($value['nu_end_ometer']);	
					// unset($value['nu_start_ometer']);	
					$value['nu_km_traveled'] = $diff;
					// $value['nu_km_traveled'] = $diff;
					...
					$value['vc_other_road_orign']       = $value['vc_orign'];
					unset($value['vc_orign']);	
					
					$value['vc_other_road_destination'] = $value['vc_destination'];
					unset($value['vc_destination']);	
					
					$value['nu_other_road_km_traveled'] = $diff;
					unset($value['nu_km_traveled']);	
						//unset($value['nu_end_ometer']);	
							//unset($value['nu_start_ometer']);	
					...
					$tmp = $this->CustomerLocationDistance->find('first', array(
													'conditions'=>array(
													'lower(CustomerLocationDistance.vc_loc_from_code)'=>strtolower(trim($value['vc_orign']))
													),
													'fields'=>array('CustomerLocationDistance.loc_from')));
								
					$value['vc_orign_name']=  $tmp == true ? $tmp['CustomerLocationDistance']['loc_from'] :  null;
								
					$tmp = $this->CustomerLocationDistance->find('first', array(
														'conditions'=>array(
														'lower(CustomerLocationDistance.vc_loc_to_code)'=>strtolower(trim($value['vc_destination']))),
														'fields'=>array('CustomerLocationDistance.loc_to')));
							

							
					$value['vc_destination_name'] = $tmp == true ? $tmp['CustomerLocationDistance']['loc_to'] :  null;
					$tmp = $this->CustomerLocationDistance->find('first', array(
													'conditions'=>array(
													'lower(CustomerLocationDistance.vc_loc_from_code)'=>strtolower(trim($value['vc_other_road_orign']))
													),
													'fields'=>array('CustomerLocationDistance.loc_from')));
								
					$value['vc_other_road_orign_name']=  $tmp == true ? $tmp['CustomerLocationDistance']['loc_from'] :  null;
								
					$tmp = $this->CustomerLocationDistance->find('first', array(
														'conditions'=>array(
														'lower(CustomerLocationDistance.vc_loc_to_code)'=>strtolower(trim($value['vc_other_road_destination']))),
														'fields'=>array('CustomerLocationDistance.loc_to')));
								
					$value['vc_other_road_destination_name'] = $tmp == true ? $tmp['CustomerLocationDistance']['loc_to'] :  null;
					
					$value['ch_road_type']= $value['selectedroad'];
					$value['dt_log_date'] = date('d-M-Y H:i:s', strtotime($value['dt_log_date']));
                    
					$value['vc_comp_code'] = trim($this->Session->read('Auth.Member.vc_comp_code'));
					$logid =$this->CustomerInactiveVehicleLog->getPrimaryKey();
                    $value['vc_log_detail_id'] =$logid; 
					
                    $value['vc_customer_no'] = trim($this->Session->read('Auth.Member.vc_username'));		
					$value['vc_remark_by'] = 'USRTYPE01';                	
					$value['vc_status'] = 'STSTY03';		
					$value['dt_created_date']=date('Y-m-d');
					unset($value['selectedroad']);
					$this->CustomerInactiveVehicleLog->set($value);
					$this->CustomerInactiveVehicleLog->save($value, false);
					
					if (isset($value['uploaddocs']['name']) && trim($value['uploaddocs']['error']) == 0) {
								...
								$filename = date('YmdHis').'-'.$value['uploaddocs']['name'];
								...
								$saveData['UploadDocumentLogInspector']['vc_uploaded_doc_type'] = $value['uploaddocs']['type'];
								
								$uploadstatus = move_uploaded_file($value['uploaddocs']['tmp_name'],$dir.DS.$filename);
									//pr($saveData);die;		
								if($uploadstatus==true){
    ...
    function downloadPaymentReceipt() {
        ...
        try {
            ...
            $columnsValues = array('Assessment
                ...
                'Vehicle License No.', 'Vehicle Register No.',
